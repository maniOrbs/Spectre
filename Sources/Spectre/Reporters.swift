...
func printFailures(_ failures: [CaseFailure]) {
    for failure in failures {
        ...
        if let contents = try? String(contentsOfFile: failure.failure.file, encoding: .utf8) as String {
...
class StandardReporter: CountReporter {
    ...
    func color(_ color: ANSI, _ message: String) {
        let indentation = String(repeating: " ", count: depth * 2)
        print("\(indentation)\(color)\(message)\(ANSI.Reset)")
    }
}
